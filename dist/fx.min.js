(function(a){var b=a.document,c=function(a,b){return new c.fn.init(a,b,c)};c.provide=function(a){a=a.split(".");var b=this,c,g;"fx"===a[0]&&(a=a.slice(1));c=a.length;for(g=0;g<c;g++)"undefined"==typeof b[a[g]]&&(b[a[g]]={}),b=b[a[g]];return b};c.getType=function(a){return c.Type.get(a)};c.hasAttribute=function(a,b){if(a.hasAttribute)return a.hasAttribute(b);a.getAttribute&&a.getAttribute(b)};c.hasPrefix=function(a,b){if("[object String]"!==c.getType(a))return!1;b=b.split("");for(var f=0;f<b.length;f++)if(a.substring(0,
...
" "),a.className=a.className.replace(/(^\s+|\s+$)/g,""))};c.swapClass=function(a,b,f){c.removeClass(a,b);c.addClass(a,f)};c.documentReady=function(a,b){var c=!1,g=!0,h=a.document,p=h.documentElement,m=h.addEventListener?"addEventListener":"attachEvent",l=h.addEventListener?"removeEventListener":"detachEvent",r=h.addEventListener?"":"on",q=function(g){if(!("readystatechange"==g.type&&"complete"!=h.readyState)&&(("load"==g.type?a:h)[l](r+g.type,q,!1),!c&&(c=!0)))b.call(a,g.type||g)},s=function(){try{p.doScroll("left")}catch(a){setTimeout(s,
50);return}q("poll")};if("complete"==h.readyState)b.call(a,"lazy");else{if(h.createEventObject&&p.doScroll){try{g=!a.frameElement}catch(t){}g&&s()}h[m](r+"DOMContentLoaded",q,!1);h[m](r+"readystatechange",q,!1);a[m](r+"load",q,!1)}};c.fn=c.prototype={constructor:c,init:function(a,c,f){if(a){if(f.isElement(a))return a}else return[];c=c||b.body;if(f.isQuery(a))a=f.Dom.find(c,a);else return f.createElement(a);return a?f.createElement(a):[]}};a.fx=c})(window);
...
fx.Vector2.TO_RADIANS=Math.PI/180;fx.Percent={of:function(a,b){return b*(a/100)},from:function(a,b){return 100*(a/b)},decimalToPercent:function(a){return 100*a},rangeToPercent:function(a,b,c){return 100*((c-a)/(b-a))},rangeToDecimal:function(a,b,c){return(b-a)/100*c}};fx.Range=function(a,b){this.start=a||0;this.end=b||1};
...
this.value.toFixed(2)+", start: "+this.start+", end: "+this.end+", step: "+this.step+", numSteps: "+this.getNumSteps().toFixed(2)+", reverse: "+this.reverse}};fx.Timer=function(a,b){if(!window.requestAnimationFrame||!window.cancelAnimationFrame)throw Error("requestAnimationFrame and cancelAnimationFrame are required for Timer");if("[object Array]"===Object.prototype.toString.apply(a)){if(2!==a.length)throw Error("Parameter 'Time' used as random range must contain excactly two numerical values: start and end");if("[object Number]"!==Object.prototype.toString.apply(a[0])||"[object Number]"!==Object.prototype.toString.apply(a[1]))throw Error("Time parameter 'min' and 'max' must be numerical");
this.min=a[0];this.max=a[1];this.isRandomize=!0}else if("[object Number]"===Object.prototype.toString.apply(a))this.time=a;else throw Error("Accepted values for parameter 'time' are Number and Array.");if(!b)throw Error("Missing required parameter 'notify'.");this.notify=b;this.tick=this.id=0;this.paused=this.running=!1;this.context=this;this.usePresicionTimer=!!window.performance.now;var c=this;window.performance=window.performance||{};performance.now=function(){return performance.now||performance.mozNow||
performance.msNow||performance.oNow||performance.webkitNow||function(){return(new Date).getTime()}}();this._getRndInterval=function(){return this.min+Math.random()*(this.max-this.min)};this._onNotify=function(){c.notify.call(c.context);c.tick++;c.isRandomize&&(clearTimeout(c.id),c.id=setTimeout(c._onNotify,c._getRndInterval()))}};
...
", tick: "+this.tick+", usePresicionTimer: "+this.usePresicionTimer}};fx.RAFTimer=function(a,b,c){if(!window.requestAnimationFrame||!window.cancelAnimationFrame)throw Error("requestAnimationFrame and cancelAnimationFrame are required for RAFTimer.");if(a){if(isNaN(a))throw Error("Parameter 'duration' is not a Number.");if(!b)throw Error("Missing required parameter 'notify'.");}else throw Error("Missing required parameter 'duration'.");this.usePresicionTimer=!!window.performance&&!!window.performance.now;window.performance=window.performance||{};performance.now=function(){return performance.now||
performance.mozNow||performance.msNow||performance.oNow||performance.webkitNow||function(){return(new Date).getTime()}}();this.duration=a;this.notify=b;this.useFrames=c||!1;this.requestId=0;this.paused=this.running=!1;this.ms=Infinity;this.count=this.currentFrame=this.currentTime=0;this.maxDeltaTime=160;var d,e=this;this._callback=function(){e.currentTime=e.duration;e.count++;e.lastTime=0;e.reset();e.notify.call(e)};this._tick=function(){if(e.running&&!e.paused){e.useFrames?e.currentFrame==e.duration&&
e._callback():e.currentTime>=e.duration&&e._callback();var a=performance.now();e.ms=a-(d||a);e.ms>e.maxDeltaTime&&(e.ms=0);d=a;e.currentTime+=e.ms;e.currentFrame++;e.requestId=requestAnimationFrame(e._tick)}}};
